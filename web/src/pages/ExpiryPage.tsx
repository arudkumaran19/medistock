...
export function ExpiryPage() {
  ...
  useEffect(() => { setIsLoading(true); setError(""); Promise.all([inventoryApi.expiring(days), inventoryApi.facilities()]).then(([rows, locations]) => { setBatches(rows); setFacilities(locations); }).catch((e: Error) => setError(e.message)).finally(() => setIsLoading(false)); }, [days]);
  ...
  const critical = batches.filter(x => new Date(x.expiryDateUtc).getTime() < Date.now()).length;
  return <main className={styles.page}>
    ...
    {isLoading && <p role="status">Loading expiry data…</p>}{error && <p className="error" role="alert">{error} <button onClick={() => { setIsLoading(true); void Promise.all([inventoryApi.expiring(days), inventoryApi.facilities()]).then(([rows, locations]) => { setBatches(rows); setFacilities(locations); setError(""); }).catch((e: Error) => setError(e.message)).finally(() => setIsLoading(false)); }} type="button">Retry</button></p>}
    {!isLoading && !error && visible.length === 0 && <p className={styles.empty}>No active batches match this expiry window and search.</p>}
    {!isLoading && !error && visible.length > 0 && <div className="table-wrap"><table><thead><tr><th>Medicine</th><th>Batch</th><th>Facility</th><th>Quantity</th><th>Expires</th><th>Alert</th><th>Action</th></tr></thead><tbody>{visible.map(batch => { const expired = new Date(batch.expiryDateUtc).getTime() < Date.now(); const daysLeft = Math.ceil((new Date(batch.expiryDateUtc).getTime() - Date.now()) / 86400000); return <tr key={batch.id}><td>{batch.medicineName}</td><td>{batch.batchNumber}</td><td>{facilities.find(f => f.id === batch.facilityId)?.name ?? "Unknown"}</td><td>{batch.quantityOnHand}</td><td>{new Date(batch.expiryDateUtc).toLocaleDateString()}</td><td><span className={`${styles.badge} ${expired ? styles.critical : styles.soon}`}>{expired ? "Expired" : daysLeft <= 30 ? "Expiring soon" : "Monitor"}</span></td><td><Link to={`/batches/${batch.id}`}>Review batch</Link></td></tr>; })}</tbody></table></div>}
  </main>;
}
