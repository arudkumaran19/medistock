...
{
    [HttpGet]
    public async Task<ActionResult<ApiResponse<IReadOnlyList<MedicineResponse>>>> GetAll(CancellationToken cancellationToken) => Ok(new ApiResponse<IReadOnlyList<MedicineResponse>>(await db.Medicines.AsNoTracking().Where(x => x.IsActive).OrderBy(x => x.Name).Select(x => new MedicineResponse(x.Id, x.Code, x.Name, x.Unit, x.MinimumStockLevel, x.IsActive)).ToListAsync(cancellationToken)));
